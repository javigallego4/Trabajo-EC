...
saveHTML(
  generate_result(),
  htmlfile = "animaciones/KMeans/kMeans.html",
  img.name = "k_means",
...
saveHTML(
  generate_result(),
  htmlfile = "animaciones/KNN/kNN.html",
  img.name = "knn",
...
Ahora, gracias al método `grad.desc` del paquete *animation* observamos la convergencia al mínimo. El archivo asociado a esta animación es **GD_convexo.html**: 

```{r}
# Convexa
saveHTML({grad.desc(fdejong, c(-10,-10,10,10), c(8,8))
},htmlfile = "animaciones/Gradiente Descendente/GD_convexo.html", img.name = "gd_convexa", imgdir = "animaciones/Gradiente Descendente/images")
```
...
A continuación, construimos dos animaciones. Cada una de ellas con un punto inicial distinto, que ha sido escogido a medida. En los archivos **GD_dos_minimos_1.html** y **GD_dos_minimos_2.html** podemos observar que la convergencia no es la misma: 

```{r}
# Convergencia a un mínimo
saveHTML({grad.desc(f1, pi * c(-2, -2, 2, 2), c(-2 * pi, 2))
},htmlfile = "animaciones/Gradiente Descendente/GD_dos_minimos_1.html", img.name = "gd_dos_minimos1", imgdir = "animaciones/Gradiente Descendente/images")

# Cambiamos el punto inicial y la convergencia ahora es a otro mínimo, distinto al anterior
saveHTML({grad.desc(f1, pi * c(-2, -2, 2, 2), c(-2 * pi, -2))
},htmlfile = "animaciones/Gradiente Descendente/GD_dos_minimos_2.html", img.name = "gd_dos_minimos2", imgdir = "animaciones/Gradiente Descendente/images")
...
},htmlfile = "animaciones/Biseccion/Biseccion.html", img.name = "biseccion", imgdir = "animaciones/Biseccion/images/")
```
...
saveHTML(generate_result(), htmlfile = "animaciones/MonteCarlo/MonteCarlo.html", img.name = "montecarlo", imgdir = "animaciones/MonteCarlo/images/")
```
...
```{r,dev='png'}
saveHTML({moving.block(dat = ObamaSpeech, FUN = function(...,dat = dat, i = i, block = block) {plot(...,x = i + 1:block, xlab = "índice del párrafo", ylim = range(dat), ylab = sprintf("ObamaSpeech[%s:%s]", i + 1, i + block))}, type = "o", pch = 20)},htmlfile = "animaciones/Obama/Obama.html", img.name = "Obama", imgdir = "animaciones/Obama/images/")
```
